...
We retain the threat model from the progress report~\cite{mid}: an honest-but-curious adversary with full visibility into the storage layer (block addresses, timestamps, read/write types) but no access to plaintext data values (AES-encrypted at rest) and no ability to modify the training computation.
...
ORAM          & Path ORAM via PyORAM~\cite{pyoram}, AES-encrypted blocks \\
...
CIFAR-10 transforms (normalize, random crop, random flip) take $<$0.05\,ms per sample---negligible relative to the $\sim$0.4\,ms per-sample ORAM read.
...
The progress report's overhead decomposition predicted per-access cost proportional to $O(\log N) \times B / S$, where $B$ is the ORAM block size and $S \approx 3072$ bytes is the serialized CIFAR-10 sample size.
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Model} & \textbf{Plaintext Total (s)} & \textbf{ORAM Total (s)} \\
\midrule
...
The ResNet-50 per-sample time is 2.7$\times$ higher than ResNet-18, but the model's forward/backward pass is $\sim$6$\times$ more expensive (measured from the plaintext times).
...
The $N=25{,}000$ data point (1,537.6\,s) is anomalous: the ORAM tree at 25K samples with 4\,KB blocks occupies $\sim$200\,MB, which should fit in memory.
...
\begin{tabular}{@{}rrrr@{}}
\toprule
\textbf{Batch Size} & \textbf{Plaintext (s)} & \textbf{ORAM (s)} & \textbf{Ratio} \\
\midrule
32  & 698.0 & 86.4 & 8.1$\times$ \\
64  & 677.5 & 54.9 & 12.3$\times$ \\
128 & 451.4 & 52.3 & 8.6$\times$ \\
256 & 416.0 & 51.5 & 8.1$\times$ \\
512 & 440.8 & 51.8 & 8.5$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The uptick at batch=512 is consistent with reduced learning-rate effectiveness at large batch sizes affecting the compute path (more gradient updates per sample at convergence).

The overhead ratio (8--12$\times$ at 5K samples) is substantially lower than the 90--180$\times$ reported in the progress report at 50K samples on CPU.
The difference has three sources: (1) smaller $N$ reduces tree height and per-access block operations, (2) MPS acceleration reduces the compute baseline (denominator), and (3) sequential phase execution warms the filesystem cache.
...
An adversary monitoring the storage layer sees exactly which sample was read, in what order, and can reconstruct the full epoch permutation.
Under curriculum learning, hard-example mining, or data-dependent sampling (which we did not use, but which are common in production pipelines), the frequency distribution becomes skewed, and the adversary learns which samples the model finds difficult.
...
ORAM optimized (ram, w=4)        & 2 & 144.2 & 25.49 \\
\bottomrule
\end{tabular}
\end{table}

The ``optimized'' configuration is slower (144.2\,s vs.\ 84.8\,s), confirming that the multi-worker IPC cost dominates any benefit from the RAM backend at this image size.
